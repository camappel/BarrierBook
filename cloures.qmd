...
#| include: true
# Faceted plot: one panel per closure showing water level, predicted tide, and closure period

import math

# Group closures by storm; for each storm, grab min start date and max end date (the 'window' for all closures in the storm)
...
for idx, storm in enumerate(storms):
    ...
    ax.plot(TSP[mask], WLP[mask], 'b', linewidth=1.5, label='Observed')
    ax.plot(TSP[mask], TIP[mask], 'r', linewidth=1.2, label='Predicted tide')
    ...
    for cl_idx, row in group.iterrows():
        ...
        ax.axvspan(cl_start, cl_end, color='m', alpha=0.25, label=label)
    # For each closure in this storm, plot the max water level marker (if available)
    for _, row in group.iterrows():
        if not pd.isnull(row['Max WL Time']):
            ax.plot(row['Max WL Time'], row['Max WL'], 'ko', label='Max WL')
    # Set y-axis limit to 3.5m for every subplot
    ax.set_ylim(top=3.8)
    # Add horizontal dashed line at 3m
    ax.axhline(3, color='black', linestyle=':', linewidth=2)
    ax.set_title(f"Storm {storm}: {storm_start:%Y-%m-%d}", fontsize=13, fontweight='bold')
...
# Remove empty subplots if nrows*ncols > n_storms
